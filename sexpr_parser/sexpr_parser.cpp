...
namespace SEXPR
{
    ...
    std::string PARSER::GetFileContents( const std::string &aFileName )
    {
        ...
        wxString line = wxString::FromUTF8( aFileName );

        if( line.IsEmpty() )  // happens when aString is not a valid UTF8 sequence
        {
            line = wxConvCurrent->cMB2WC( aFileName.c_str() );    // try to use locale conversion

            if( line.IsEmpty() )
                line = wxString::From8BitData( aFileName.c_str() );    // try to use native string
        }
        wxString fname = line;
