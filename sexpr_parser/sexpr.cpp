...
namespace SEXPR
{
    ...
    {
        // 第一步：重新排列 vector，将需要删除的元素（在这里是 nullptr）移动到末尾
        // std::remove 的返回值是一个新的迭代器，指向第一个不想要删除的元素
        auto newEnd = std::remove(m_children.begin(), m_children.end(), nullptr);

        // 第二步：从 vector 中删除这些元素，从 newEnd 到 vector 的真实末尾
        // 由于 std::remove 已经将元素移动到了 vector 的末尾，erase 将会删除它们
